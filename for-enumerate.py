...
while x < qtd:
    ...
    poltronas = int(input(f'Qualntas poltronas na sala {x+1}: '))
...
while True:
    ...
    if sala == 0:
        print('Saindo...')
        break
    if sala > len(qtdSalas) or sala < 1:
        print('Sala inválida')
    elif qtdSalas[sala - 1] == 0:
        print('Desculpe, sala lotado!')
    else:
        lugares = int(input(f'Quantos lugares você deseja ({qtdSalas[sala - 1]} vagos):'))
        if lugares > qtdSalas[sala - 1]:
            print('Esse número de lugares não está disponível!')
        elif lugares < 0:
            print('Número inválido')
        else:
            qtdSalas [sala - 1] -= lugares
            comprados[sala -1 ] += lugares
            vendas_total += lugares
            print(f'{lugares} lugares vendidos')
print('Utilização das salas')
for sala, vagos in enumerate(qtdSalas):
    print(f'Sala {sala+1} - {vagos} lugar(es) vazio(s)')
for sala, comprados in enumerate(comprados):
    print(f'Ingressos vendidos\n Sala {sala+1} - {comprados}')
print(f'Total de ingressos vendidos: {vendas_total}') 
